...
import { IconFile, IconX, IconEye, IconFileText, IconCheck, IconPencil, IconPlus, IconLoader2, IconSearch } from "@tabler/icons-react";
...
export function EmployeeFilesTab({ formData, handleChange, disabled }: EmployeeFilesTabProps) {
            ...
            <Card className="border-none shadow-none bg-muted/50 rounded-[2rem] overflow-hidden">
                <CardHeader className="p-8 pb-4 flex flex-col xl:flex-row xl:items-center justify-between gap-6">
                    <div className="flex items-center gap-4">
                        <div className="h-12 w-12 rounded-3xl bg-blue-500/10 flex items-center justify-center text-blue-600 shadow-inner">
                            <IconFile className="h-6 w-6" />
                        </div>
                        <div className="space-y-0.5">
                            <CardTitle className="text-xl md:text-2xl font-black tracking-tight text-foreground">Employee Documents</CardTitle>
                ...
                <CardContent className="p-8">
                    {filteredFiles.length > 0 ? (
                                ...
                                {filteredFiles.map((file) => (
                                    ...
                                    >
                                        <div className="flex items-center gap-4 overflow-hidden w-full">
                                            <div className="h-12 w-12 rounded-xl bg-muted flex items-center justify-center shrink-0 text-blue-500 shadow-inner transition-transform group-hover:scale-105">
                        ...
                        <div className="py-16 text-center border-2 border-dashed border-border rounded-[2rem] bg-card/50">
                            ...
                            <h3 className="text-lg md:text-xl font-black text-foreground mb-2">No documents yet</h3>
                            <p className="text-xs md:text-sm text-muted-foreground font-medium mb-8 max-w-sm mx-auto">Upload NIC copies, certificates, and contracts here.</p>
                            {!showUpload && !disabled && (
                                <Button
                                    onClick={() => setShowUpload(true)}
