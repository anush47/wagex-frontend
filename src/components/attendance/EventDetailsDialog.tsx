...
const AttendanceMap = dynamic(() => import("@/app/[locale]/(protected)/employer-portal/companies/[id]/policies/components/policy/AttendanceMap"), {
    ssr: false,
                        ...
                        <div className="space-y-2">
                            ...
                            <div className="bg-muted/30 p-3 rounded-xl border border-border/50 relative overflow-hidden group">
                                <div className="relative z-10">
                                    ...
                                    <div className="flex items-center gap-2 mt-1">
                                        <Badge variant="outline" className={cn(
                                            ...
                                            event.source === 'PORTAL' ? "bg-green-500/20 text-green-600" : "bg-blue-500/20 text-blue-600"
                                        )}>
                                            {event.source}
                                        </Badge>
                                        {event.source === 'PORTAL' && (
                                            <Badge variant="outline" className="text-[9px] uppercase font-black px-1.5 py-0 bg-primary/10 text-primary border-none">
                                                Secured
                                            </Badge>
                                        )}
                                    </div>
                                ...
                                <div className="absolute right-[-10px] bottom-[-10px] opacity-10 group-hover:scale-125 transition-transform">
                                    <IconDeviceMobile className="h-12 w-12" />
                                </div>
                            </div>
                        </div>
                        <div className="space-y-2">
                            ...
                            <div className="bg-muted/30 p-3 rounded-xl border border-border/50">
                                <p className="text-xs font-bold">{event.apiKeyName || "Portal Session"}</p>
                                <p className="text-[10px] text-muted-foreground mt-0.5">{event.apiKeyName ? "External Auth" : "Authenticated User"}</p>
                            </div>
                        </div>
                    </div>

                    {/* Biometric / Fingerprint Verification Status */}
                    {event.source === 'PORTAL' && (
                        <div className="p-4 rounded-2xl bg-green-50/50 dark:bg-green-500/5 border border-green-200/50 dark:border-green-500/20 flex items-center justify-between">
                            <div className="flex items-center gap-3">
                                <div className="h-10 w-10 rounded-xl bg-green-500 text-white flex items-center justify-center shadow-lg shadow-green-500/20">
                                    <IconFingerprint className="h-6 w-6" />
                                </div>
                                <div>
                                    <p className="text-[11px] font-black uppercase tracking-tight text-green-700 dark:text-green-400">Verified Identity</p>
                                    <p className="text-[10px] font-medium text-green-600/70">Authenticated via Secure Portal Session</p>
                                </div>
                            </div>
                            <Badge className="bg-green-500/20 text-green-700 border-none px-3 py-1 font-black text-[10px] uppercase">
